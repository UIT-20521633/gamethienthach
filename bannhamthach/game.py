...
pygame.display.set_caption("Game Bắn Thiên Thạch")
##Tạo icon game
    ...
    for event in pygame.event.get():
        ...
        if running :
            ...
            if event.type == meteor_time:
                # Random vị trí 
                x_pos=random.randint(-100,1380)
                y_pos=random.randint(-100,-50)
                meteor_rect=meteor.get_rect(center=(x_pos,y_pos))
                positions=pygame.math.Vector2(random.uniform(-0.5,0.5),1)
            ...
            if(bg_y==800):
                bg_y=0
            screen.blit(bg_chinh,(0,bg_y-800),)  
